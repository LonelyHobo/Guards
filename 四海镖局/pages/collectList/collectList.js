      ...
      success: function (res) {
        ...
        if (res.statusCode == 200) {
          ...
          if (code == 'Merchant'){
            ...
            data.result.forEach(function (value, index) {
              if (value.Service.PicUrl && value.Service.PicUrl != null) {
                value.Service.PicUrl = vm.data.api + value.Service.PicUrl;
              } else {
                value.Service.PicUrl = '/images/imgNull.png';
              }
              var ServiceiD = value.Service.ServiceID;
              var introduce = value.Service.Introduce;
              var nodes = WxParse.wxParse('introduce2' + index, 'html', introduce, vm, 5);
              data.result[index].Service['nodes'] = nodes;
            });
          }
          vm.setData({
